import React from "react";
import "../App.css";
import Button from "../component/Button";
import Logo from "../assets/blackLogo.avif";
import { Link } from "react-router-dom";
import facebook from "../assets/facebook.png";
import insta from "../assets/insta.png";
import twitter from "../assets/twitter.png";
import youtube from "../assets/youtube.png";

import footerimg1 from "../assets/footerimg1.png";
import footerimg2 from "../assets/footerimg2.png";
import footerimg3 from "../assets/footerimg3.png";
import footerimg4 from "../assets/footerimg4.png";
import footerimg5 from "../assets/footerimg5.png";
import footerimg6 from "../assets/footerimg6.png";

const Footer = () => {
  const Link = [
    {
      id: 1,
      icon: facebook,
    },
    {
      id: 2,
      icon: insta,
    },
    {
      id: 3,
      icon: twitter,
    },
    {
      id: 4,
      icon: youtube,
    },
  ];

  const FooterImg = [
    {
      id: 1,
      img: footerimg1,
    },
    {
      id: 2,
      img: footerimg2,
    },
    {
      id: 3,
      img: footerimg3,
    },
    {
      id: 4,
      img: footerimg4,
    },
    {
      id: 5,
      img: footerimg5,
    },
    {
      id: 6,
      img: footerimg6,
    },
  ];

  return (
    <div className=" text-white   w-full text-center">
      <div className="gradient-bg text-white py-4 px-3 w-full text-center">
        <div className="max-w-[1200px] mx-auto flex-col md:flex-row gap-8 flex items-center py-10 md:py-14 lg:py-20 justify-between">
          <h2 className="text-2xl md:text-5xl font-semibold">
            Need a plumber fast?
          </h2>

          <Button children="Call Us 555-0123" className="py-3 px-4" />
        </div>
      </div>

      <div className="max-w-[1200px] mx-auto grid gap-6 grid-cols-1 gird-rows-4 md:grid-cols-2 lg:grid-rows-1 lg:grid-cols-4 grid-rows-2  items-center px-3 py-10 md:py-14 lg:py-20 justify-between">
        <div className="w-full">
          <img src={Logo} alt="" className="h-6" />
          <p className="text-gray-400 text-start  mt-2 w-full">
            Top-notch residential and commercial plumbing servic
          </p>
          <div className="flex items-center gap-4 mt-3">
            {Link.map((icon, index) => (
              <img
                src={icon.icon}
                key={index}
                alt=""
                className="h-4 md:h-6 hover:translate-1 transform-all duration-300"
              />
            ))}
          </div>
        </div>

        <div className="grid max-w-[300px] w-full grid-rows-2 grid-cols-3 gap-2">
          {FooterImg.map((img) => (
            <img
              src={img.img}
              className="h-20 rounded-md object-cover"
              alt=""
            />
          ))}
        </div>
        <div className="text-black text-start flex flex-col gap-3 lg:gap-6">
          <h5 className="font-semibold text-[24px]">Manhattan</h5>
          <span className="text-gray-500 text-md font-semibold">
            1234 Broadway New York, NY 1001
          </span>
          <p>Stop by and meet our team we're happy to help!</p>
        </div>
        <div className="text-black text-start flex flex-col gap-3 lg:gap-6">
          <h5 className="font-semibold text-[24px]">Brooklyn</h5>
          <span className="text-gray-500 text-md font-semibold">
            1234 Broadway New York, NY 1001
          </span>
          <p>Open Monday to Saturday, 9:00 AM – 6:00 PM</p>
        </div>
      </div>
    </div>
  );
};

export default Footer;
